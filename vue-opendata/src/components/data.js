...
export async function crimesNYC(county, offense_category, crime_category) {
  ...
  crime.value = data
    ...
    .filter(
      (item) =>
        offense_category == 'ALL' ||
        item.offense_category.toLowerCase() == offense_category.toLowerCase(),
    )
    .filter(
      (item) =>
        crime_category == 'ALL' ||
        item.law_code_category_description.toLowerCase() == crime_category.toLowerCase(),
    )
}
